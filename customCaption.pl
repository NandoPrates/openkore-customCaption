...
sub setTitle {
	...
	if ($net->getState() == Network::IN_GAME) {
		...
		my $cambio;
		if ($config{master} =~ /Odin/) {
			$cambio = 0.80;
		} elsif ($config{master} =~ /Asgard/) {
			$cambio = 0.97;
		} elsif ($config{master} =~ /Thor/) {
			$cambio = 0.28;
		}
		$moneyIHave = formatNumber(sprintf("%.2f", (0.97/1000000)*$char->{zeny}));
		if ($w_sec > 0) {
		...
		$args->{return} = swrite("@<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< @<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<",
		[sprintf("[%s]%s (%s)", substr($servers[$config{'server'}]{'name'}, 0, 1), $config{'username'}, $char->{name}),
		sprintf("L%s w%s %s %sz (%sz)",
									$char->{'lv'},
									$w,
									$fieldx,
									formatNumber($char->{'zeny'}),
		...
		#[%server] %conf{username} (%n) : %basel/%jobl : p%w %map(%pos)
	} elsif ($net->getState() == Network::NOT_CONNECTED) {
		# Translation Comment: Interface Title
		$args->{return} = sprintf("offline - [%s]%s", (substr($servers[$config{'server'}]{'name'}, 0, 1) || '?'), ($config{'username'} || '??'));
	} else {
		# Translation Comment: Interface Title
		$args->{return} = sprintf("[%s]%s - Connecting", (substr($servers[$config{'server'}]{'name'}, 0, 1) || '?'), ($config{'username'} || '??'));
	}
}
